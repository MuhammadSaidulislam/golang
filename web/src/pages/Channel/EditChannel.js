...
const EditChannel = (props) => {
  ...
  return (
    <>
      <Modal show={props.channelModal} onHide={props.channelModalClose} centered size="lg">
        <div className='modalHeading'>
          <h1>{isEdit ? t('更新渠道信息') : t('创建新的渠道')}</h1>
          <button onClick={props.channelModalClose}><IconClose /></button>
        </div>
          ...
          <div className='channelBox'>
            <div className='tokenFirst'>
              <div className="personalInput w-100">
                <label>{t('名称')}</label>
                <input type="text" className="search-input" placeholder={t('请为渠道命名')} name='name' value={inputs.name} onChange={(value) => handleInputChange('name', value.target.value)} />
              </div>

              {inputs.type !== 3 && inputs.type !== 8 && inputs.type !== 22 && inputs.type !== 36 && <div className="personalInput w-100">
                <label>{t('代理')}</label>
                <input type="text" className="search-input" placeholder={t('此项可选，用于通过代理站来进行 API 调用')} name='base_url' value={inputs.base_url} onChange={(value) => handleInputChange('base_url', value.target.value)} />
              </div>}
              ...
              <div className="personalInput w-100 mt-3">
                <label>{t('默认测试模型')}</label>
                <input type="text" className="search-input" placeholder={t('输入自定义模型名称')} value={customModel} onChange={(e) => setCustomModel(e.target.value)} />
                <button className='searchBtn' onClick={addCustomModels}>{t('填入')}</button>
              </div>

              {inputs.type === 1 && <div className="personalInput w-100 mt-3">
                <label>{t('组织，可选，不填则为默认组织')}</label>
                <input type="text" className="search-input" placeholder={t('请输入组织org-xxx')} name='openai_organization' value={inputs.openai_organization} onChange={(value) => handleInputChange('openai_organization', value.target.value)} />
              </div>}
              <div className="personalInput w-100">
                <label>{t('默认测试模型')}</label>
                <input type="text" className="search-input" placeholder={t('不填则为模型列表第一个')} name='test_model' value={inputs.test_model} onChange={(value) => handleInputChange('test_model', value.target.value)} />
              </div>
              <div className="personalInput w-100">
                <label>{t('渠道标签')}</label>
                <input type="text" className="search-input" placeholder={t('渠道标签')} name='tag' value={inputs.tag} onChange={(value) => handleInputChange('tag', value.target.value)} />
              </div>
              <div className="tagChannel">
                <div className="personalInput w-100">
                  <label>{t('渠道优先级')}</label>
                  <input type="text" className="search-input" placeholder={t('渠道优先级')} name="priority" value={inputs.priority}
                    onChange={(e) => {
                ...
                <div className="personalInput w-100">
                  <label>{t('渠道权重')}</label>
                  <input type="text" className="search-input" placeholder={t('渠道权重')} name="weight" value={inputs.weight}
                    onChange={(e) => {
                      const value = e.target.value;
                      const number = parseInt(value, 10);
                      if (isNaN(number)) {
            ...
            <div className='tokenFirst'>
              <div className="personalInput w-100">
                <Switch
                  ...
                  uncheckedText=""
                  style={{
                    backgroundColor: autoBan ? '#dbeafe' : '#f1f5f9',
                    border: 'none',
                  }}
                  innerStyle={{
                ...
                />
                <label>{t('是否自动禁用（仅当自动禁用开启时有效），关闭后不会自动禁用该渠道：')}</label>
              </div>
              {!isEdit && <div className="personalInput w-100">
                <label>{t('批量创建')}</label>
                <Switch
                  checked={batch}
                  ...
                  uncheckedText=""
                  style={{
                    backgroundColor: batch ? '#dbeafe' : '#f1f5f9',
                    border: 'none',
                  }}
                  innerStyle={{
                    backgroundColor: '#cbd5e1',
              ...
              <div className="personalInput w-100 mt-3">
                ...
                <button className='searchBtn mt-3' onClick={() => { handleInputChange('model_mapping', JSON.stringify(MODEL_MAPPING_EXAMPLE, null, 2)); }}>{t('填入模板')}</button>
              </div>
              <div className="personalInput w-100 mt-3">
                <label>{t('状态码复写（仅影响本地判断，不修改返回到上游的状态码）')}</label>
                <TextArea
                  ...
                  autoComplete="new-password"
                />
                <button className='searchBtn mt-3' onClick={() => { handleInputChange('status_code_mapping', JSON.stringify(STATUS_CODE_MAPPING_EXAMPLE, null, 2)); }}>{t('填入模板')}</button>
              </div>
              {inputs.type === 3 && (
                <>
