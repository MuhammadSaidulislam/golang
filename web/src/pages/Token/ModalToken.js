...
const ModalToken = ({ updateShow, handleUpdateClose, refresh, editingToken, visiable }) => {
    ...
    const loadToken = async () => {
        setLoading(true);
        let res = await API.get(`/api/token/${editingToken.id}`);
        const { success, message, data } = res.data;
        if (success) {
    ...
    useEffect(() => {
        setIsEdit(editingToken.id !== undefined);
    }, [editingToken.id]);
